...
try {
    ...
    Write-Host "[OK] Health Check: $($healthResponse.StatusCode) - $($healthResponse.Content)" -ForegroundColor Green
} catch {
...
try {
    ...
    Write-Host "[OK] Root API: $($rootResponse.StatusCode)" -ForegroundColor Green
} catch {
    if ($_.Exception.Response.StatusCode -eq 404) {
        Write-Host "[OK] Root API: 404 (Expected)" -ForegroundColor Green
    } else {
        Write-Host "[FAIL] Root API Failed: $($_.Exception.Message)" -ForegroundColor Red
    }
}
...
try {
    ...
    Write-Host "[OK] Auth Login: $($loginResponse.StatusCode)" -ForegroundColor Green
} catch {
    if ($_.Exception.Response.StatusCode -eq 500) {
        Write-Host "[OK] Auth Login: 500 (Expected - Database not connected)" -ForegroundColor Green
    } else {
        Write-Host "[FAIL] Auth Login Failed: $($_.Exception.Message)" -ForegroundColor Red
    }
}
...
try {
    ...
    Write-Host "[OK] Users: $($usersResponse.StatusCode)" -ForegroundColor Green
} catch {
    if ($_.Exception.Response.StatusCode -eq 401) {
        Write-Host "[OK] Users: 401 (Expected - Requires Authentication)" -ForegroundColor Green
    } else {
        Write-Host "[FAIL] Users Failed: $($_.Exception.Message)" -ForegroundColor Red
    }
}
...
try {
    ...
    Write-Host "[OK] Companies: $($companiesResponse.StatusCode)" -ForegroundColor Green
} catch {
    if ($_.Exception.Response.StatusCode -eq 401) {
        Write-Host "[OK] Companies: 401 (Expected - Requires Authentication)" -ForegroundColor Green
    } else {
        Write-Host "[FAIL] Companies Failed: $($_.Exception.Message)" -ForegroundColor Red
    }
}
...
try {
    ...
    Write-Host "[OK] Jobs: $($jobsResponse.StatusCode)" -ForegroundColor Green
} catch {
    if ($_.Exception.Response.StatusCode -eq 401) {
        Write-Host "[OK] Jobs: 401 (Expected - Requires Authentication)" -ForegroundColor Green
    } else {
        Write-Host "[FAIL] Jobs Failed: $($_.Exception.Message)" -ForegroundColor Red
    }
}
...
try {
    ...
    Write-Host "[OK] Search: $($searchResponse.StatusCode)" -ForegroundColor Green
} catch {
    if ($_.Exception.Response.StatusCode -eq 500) {
        Write-Host "[OK] Search: 500 (Expected - Database not connected)" -ForegroundColor Green
    } else {
